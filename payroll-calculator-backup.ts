    ...
    for (const record of attendanceRecords) {
        ...
        if (record.status === "absent") {
            ...
            if (record.leaveType === "special") {
                ...
                // Unapproved/Unpaid regular leave
                applyOccasionDeduction(1, "leave", true);
            }
            // Approved paid regular leaves (casual/annual) => no deductions
            ...
            // Deduct from Basic (standard behavior)
            const basicDeduction = perHourBasic * shortHours;
            adjustments["basicSalary"] += basicDeduction;
            totalAbsentDeduction += basicDeduction;

            // Determine which flexible rules to apply
            // If we don't have the exact split, we apply the deduction if either is true 
            // but only once for the total shortHours to avoid double penalty.
            // A more complex system would parse check-in/out times to split specifically.
            if (record.isLate) {
                applyOccasionDeduction(shortHours, "lateArrival");
            } else if (record.earlyDepartureStatus !== "none") {
