...
function EmployeePayrollHistory() {
  ...
  const stats = {
    totalPaid: history.reduce(
      (acc, curr) => acc + parseFloat(curr.netSalary),
      0,
    ),
    avgSalary:
      history.length > 0
        ? history.reduce((acc, curr) => acc + parseFloat(curr.netSalary), 0) /
          history.length
        : 0,
    totalDeductions: history.reduce(
      (acc, curr) => acc + parseFloat(curr.totalDeductions),
      0,
    ),
  };
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button
          variant="outline"
          size="sm"
          className="h-9 gap-2 text-[10px] font-bold uppercase tracking-widest shadow-xs"
        >
