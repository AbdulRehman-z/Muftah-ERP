                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <Card className="overflow-hidden border-none  ring-1 ring-border/50">
                        <CardHeader className="bg-muted/30 pb-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <CardTitle className="text-xl">Chemical Formulation</CardTitle>
                                        <CardDescription>BOM for {recipe.batchSize}{recipe.batchUnit} batch</CardDescription>
                                    </div>
                                </div>
                        ...
                        <CardContent className="p-0">
                                ...
                                {recipe.ingredients.map((ing, i) => (
                                    <div key={i} className="flex items-center justify-between p-4 hover:bg-muted/10 transition-colors">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="font-semibold text-sm">{ing.chemical.name}</p>
                                                <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                                                    Cost: Rs. {formatCurrency(ing.chemical.costPerUnit)} / {ing.chemical.unit}
                                                </p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-indigo-600">{ing.quantityPerBatch} {ing.chemical.unit}</p>
                                            <p className="text-[10px] text-muted-foreground uppercase tracking-tight mt-0.5">
                                                Rs. {formatCurrency(Number(ing.quantityPerBatch) * Number(ing.chemical.costPerUnit || 0))} Total
                                            </p>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="p-4 bg-muted/20 flex justify-between items-center font-bold text-sm">
                                <p className="text-muted-foreground uppercase tracking-widest text-[10px]">Ingredients Subtotal</p>
                                <p className="text-lg">Rs. {formatCurrency(recipe.estimatedIngredientsCost)}</p>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="overflow-hidden border-none  ring-1 ring-border/50">
                        <CardHeader className="bg-muted/30 pb-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <CardTitle className="text-xl">Packaging & Materials</CardTitle>
                                        <CardDescription>Container and distribution components</CardDescription>
                                    </div>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="p-0">
                            <div className="divide-y">
                                ...
                                <div className="flex items-center justify-between p-4 bg-background">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <p className="font-semibold text-sm">{recipe.containerPackaging.name}</p>
                                                <Badge variant="outline" className="text-[9px] uppercase font-bold py-0 h-4 border-amber-200 text-amber-700 bg-amber-50">Primary</Badge>
                                            </div>
                                            <p className="text-xs text-muted-foreground mt-0.5">
                                                {recipe.fillAmount}{recipe.fillUnit} Fill Amount
                                            </p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold">{recipe.targetUnitsPerBatch} Units</p>
                                        <p className="text-[10px] text-muted-foreground uppercase tracking-tight mt-0.5">
                                            Rs. {formatCurrency(recipe.containerPackaging.costPerUnit)} / unit
                                        </p>
                                    </div>
                                </div>
                                ...
                                {recipe.cartonPackaging && (
                                    <div className="flex items-center justify-between p-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <p className="font-semibold text-sm">{recipe.cartonPackaging.name}</p>
                                                    <Badge variant="outline" className="text-[9px] uppercase font-bold py-0 h-4 border-orange-200 text-orange-700 bg-orange-50">Master</Badge>
                                                </div>
                                                <p className="text-xs text-muted-foreground mt-0.5">
                                        ...
                                        <div className="text-right">
                                            <p className="font-bold">{recipe.containersPerCarton ? Math.floor(recipe.targetUnitsPerBatch / recipe.containersPerCarton) : 0} Buckets</p>
                                            <p className="text-[10px] text-muted-foreground uppercase tracking-tight mt-0.5">
                                                Rs. {formatCurrency(recipe.cartonPackaging.costPerUnit)} / bucket
                                            </p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {recipe.packaging.map((pkg, i) => (
                                    <div key={i} className="flex items-center justify-between p-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <p className="font-semibold text-sm">{pkg.packagingMaterial.name}</p>
                                                    <Badge variant="outline" className="text-[9px] uppercase font-bold py-0 h-4">Sticker</Badge>
                                                </div>
                                                <p className="text-xs text-muted-foreground mt-0.5">
                                        ...
                                        <div className="text-right">
                                            <p className="font-bold">{(Number(pkg.quantityPerContainer) * recipe.targetUnitsPerBatch).toLocaleString()} units</p>
                                            <p className="text-[10px] text-muted-foreground uppercase tracking-tight mt-0.5">
                                                Rs. {formatCurrency(pkg.packagingMaterial.costPerUnit)} / unit
                                            </p>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="p-4 bg-muted/20 flex justify-between items-center font-bold text-sm">
                                <p className="text-muted-foreground uppercase tracking-widest text-[10px]">Packaging Subtotal</p>
                                <p className="text-lg">Rs. {formatCurrency(recipe.estimatedPackagingCost)}</p>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <Card className="border-none  ring-1 ring-border/50 overflow-hidden">
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-sm font-semibold capitalize bg-muted/40 p-2 rounded-md border border-border/50">
                                    {recipe.containersPerCarton ? `${recipe.containersPerCarton} Per Bucket` : "Loose Units Only"}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-none  ring-1 ring-border/50 ">
                        ...
                        <CardContent className="text-xs text-primary/80 leading-relaxed italic">
                            This recipe configuration defines the Bill of Materials for a single production batch.
                            Estimated costs are based on current inventory pricing.
                        </CardContent>
                    </Card>
                </div>
            </div>
