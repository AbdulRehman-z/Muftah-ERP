...
export const EmployeesTable = ({ data }: Props) => {
    ...
    const columns: ColumnDef<Employee>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => <span>PKR {parseFloat(row.original.standardSalary || "0").toLocaleString()}</span>,
        },
        {
            accessorKey: "status",
            header: "Status",
            cell: ({ row }) => {
                const status = row.original.status as "active" | "on_leave" | "terminated" | "resigned";
                ...
                return (
                    <Badge variant="outline" className={`px-2 py-0.5 text-[10px] uppercase tracking-wider ${config.class}`}>
                        {config.label}
                    </Badge>
                );
            },
                        ...
                        <AlertDialog>
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        This will permanently delete the employee "{employee.firstName} {employee.lastName}".
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                    ...
                                    <AlertDialogAction
                                        ...
                                        onClick={() => deleteMutate.mutate({ data: { id: employee.id } })}
                                    >
                                        Delete
                                    </AlertDialogAction>
                                </AlertDialogFooter>
