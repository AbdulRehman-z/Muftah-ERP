...
const base: React.CSSProperties = { border: B, padding: "3px 7px", fontSize: 11, verticalAlign: "middle" };
const styles = {
    ...
    num: { ...base, textAlign: "right" as const, fontVariantNumeric: "tabular-nums" } as React.CSSProperties,
    totalLbl: { ...base, fontWeight: 700, textAlign: "right" as const, background: "#f9fafb" } as React.CSSProperties,
    totalNum: { ...base, fontWeight: 700, textAlign: "right" as const, background: "#f9fafb", fontVariantNumeric: "tabular-nums" } as React.CSSProperties,
    netLbl: { ...base, fontWeight: 800, background: "#bfdbfe", fontSize: 12 } as React.CSSProperties,
    netNum: { ...base, fontWeight: 800, background: "#bfdbfe", fontSize: 12, textAlign: "right" as const, fontVariantNumeric: "tabular-nums" } as React.CSSProperties,
};
...
export const PayslipView = ({ payslip, showActions = true }: PayslipViewProps) => {
    ...
    const bradfordColor =
        effectiveBradford === 0 ? "#15803d" :
            effectiveBradford < 50 ? "#a16207" :
                effectiveBradford < 250 ? "#c2410c" : "#b91c1c";

    // ── Earnings ───────────────────────────────────────────────────────────
    const dynamicAllowances = Object.entries(payslip.allowanceBreakdown || {})
        .filter(([key]) => key !== "basicSalary" && key !== "nightShift")
        .map(([key, val]) => ({ label: allowanceLabels[key] || `${key} Allowance`, value: Number(val) }))
        .filter(e => e.value > 0);
    ...
    const earnings: { label: string; value: number }[] = [
        ...
        ...(toN(payslip.incentiveAmount) + toN(payslip.bonusAmount) > 0
            ? [{ label: "Incentive / Bonus", value: toN(payslip.incentiveAmount) + toN(payslip.bonusAmount) }] : []),
    ];
            ...
            <div ref={printRef} style={{ padding: "24px 28px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" style={{ color: "#eab308", flexShrink: 0 }}>
                            <path d="M12 2L3 7v5c0 5.25 3.75 10.15 9 11.35C17.25 22.15 21 17.25 21 12V7l-9-5z"
                                fill="#fef9c3" stroke="#eab308" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                            <path d="M9 12l2 2 4-4" stroke="#eab308" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        <div>
                            <div style={{ fontSize: 15, fontWeight: 800, textTransform: "uppercase", letterSpacing: 0.5 }}>
                                TITAN ENTERPRISE
                            </div>
                            <div style={{ fontSize: 10, color: "#6b7280", fontStyle: "italic" }}>Confidential Payslip</div>
                        </div>
                    </div>
                    <div style={{ textAlign: "right", fontSize: 10, color: "#374151", lineHeight: 1.8 }}>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style={styles.data} colSpan={2}>{employee.employeeCode}</td>
                            <td style={styles.label}>Name</td>
                            <td style={styles.data} colSpan={2}>{employee.firstName} {employee.lastName}</td>
                        </tr>
                        <tr>
                            <td style={styles.label}>Designation</td>
                            <td style={styles.data} colSpan={2}>{employee.designation}</td>
                            <td style={styles.label}>CNIC</td>
                            <td style={styles.data} colSpan={2}>{employee.cnic || "N/A"}</td>
                        </tr>
                        <tr>
                            <td style={styles.label}>Bank Account Number</td>
                            <td style={styles.data} colSpan={2}>{employee.bankAccountNumber || "N/A"}</td>
                            <td style={styles.label}>Bradford Factor Period</td>
                            <td style={styles.data} colSpan={2}>{bradfordPeriod}</td>
                        </tr>
                        <tr>
                            <td style={styles.label}>Bank Name</td>
                            <td style={styles.data} colSpan={2}>{employee.bankName || "Cash"}</td>
                            <td style={styles.label}>Bradford Factor</td>
                            <td style={{ ...styles.data, fontWeight: 700, color: bradfordColor }} colSpan={2}>
                                {effectiveBradford}
                                {payslip.bradfordFactorOverride != null && (
                                    <span style={{ fontSize: 9, marginLeft: 4, color: "#9ca3af", fontWeight: 400 }}>(override)</span>
                                )}
                                {/* Edit button intentionally omitted from printRef — it's outside the popup HTML */}
                                {showActions && (
                                    <button
                                        type="button"
                        ...
                        {/* Attendance */}
                        <tr>
                            <td colSpan={6} style={{ ...styles.data, paddingTop: 5, paddingBottom: 5 }}>
                                <strong>Attendance: </strong>
                                <span style={{ marginRight: 14 }}>Present: <strong>{payslip.daysPresent ?? 0}</strong></span>
                                <span style={{ marginRight: 14 }}>Absent: <strong>{payslip.daysAbsent ?? 0}</strong></span>
                                <span style={{ marginRight: 14 }}>Leave: <strong>{payslip.daysLeave ?? 0}</strong></span>
                                <span style={{ marginRight: 14 }}>OT (Hrs): <strong>{payslip.totalOvertimeHours ?? "0.00"}</strong></span>
                        ...
                        {ep.map((earning, i) => {
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td style={styles.num}>{earning ? fmt(earning.value) : ""}</td>
                        ...
                        {/* Totals */}
                        <tr>
                            <td colSpan={2} style={styles.totalLbl}>Total Earnings</td>
                            <td style={styles.totalNum}>{fmt(totalEarnings)}</td>
                            <td colSpan={2} style={styles.totalLbl}>Total Deduction</td>
                            <td style={styles.totalNum}>{fmt(totalDedns)}</td>
                        </tr>

                        {/* Net Pay */}
                        <tr>
                            <td colSpan={4} style={{ ...styles.netLbl, textAlign: "center", fontSize: 13 }}>Net Pay</td>
                            <td style={{ ...styles.netNum, fontSize: 13 }}>{fmt(netPay)}</td>
                            <td style={{ ...styles.netLbl, textAlign: "center", fontSize: 13 }}>PKR</td>
                        </tr>
                ...
                {/* Remarks */}
                <div style={{ border: B, padding: "8px 10px", minHeight: 54, marginTop: 16, marginBottom: 32, fontSize: 11 }}>
                    {payslip.remarks || "Salaries are paid as per company policy."}
                </div>

                {/* Signatures */}
                <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 36, paddingLeft: 8, paddingRight: 8 }}>
                    {["Employee Signature", "HR / Finance Manager"].map(label => (
                        <div key={label} style={{ textAlign: "center", width: 200 }}>
                            <div style={{ borderTop: "1px solid #1f2937", paddingTop: 6, fontSize: 11, fontWeight: 700 }}>
                                {label}
                            </div>
                        </div>
                    ))}
                ...
                {/* Footer */}
                <div style={{ marginTop: 20, borderTop: "1px solid #f3f4f6", paddingTop: 10, textAlign: "center", fontSize: 9, color: "#9ca3af", textTransform: "uppercase", letterSpacing: 2 }}>
