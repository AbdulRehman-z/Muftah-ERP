    ...
    const columns: ColumnDef<EmployeeWithAttendance>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                if (!record) return <Badge variant="secondary" className="opacity-50">Not Set</Badge>;

                const statusMap = {
                    present: { label: "Present", variant: "default" as const, class: "bg-emerald-500 hover:bg-emerald-600" },
                    absent: { label: "Absent", variant: "destructive" as const, class: "" },
                    leave: { label: "Leave", variant: "secondary" as const, class: "bg-indigo-500 text-white hover:bg-indigo-600" },
                    half_day: { label: "Half Day", variant: "outline" as const, class: "border-amber-500 text-amber-500 hover:bg-amber-50" },
                    holiday: { label: "Holiday", variant: "outline" as const, class: "border-blue-500 text-blue-500 hover:bg-blue-50" },
                };

                const config = statusMap[record.status];
                return (
                    <div className="flex items-center gap-2">
                        <Badge variant={config.variant} className={config.class}>{config.label}</Badge>
                        {record.isLate && (
                            <Badge variant="outline" className="border-rose-500 text-rose-500 bg-rose-50 border-dashed animate-pulse">
                                Late
                            </Badge>
                        )}
                    </div>
        ...
        },
        {
            ...
            cell: ({ row }) => {
                const record = row.original.attendance[0];
                if (!record || record.status === "absent" || record.status === "holiday") return "-";

                return (
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-xs font-medium">
                            <span className="w-4 text-muted-foreground text-[10px] uppercase">S1</span>
                        ...
                        {(record.checkIn2 || record.checkOut2) && (
                            <div className="flex items-center gap-2 text-xs font-medium">
                                <span className="w-4 text-muted-foreground text-[10px] uppercase">S2</span>
                                <span className="text-emerald-600 font-bold">{record.checkIn2 || "??:??"}</span>
                                <span className="text-muted-foreground opacity-50">→</span>
                                <span className="text-rose-600 font-bold">{record.checkOut2 || "??:??"}</span>
                            </div>
                        )}
                    </div>
                );
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-2">
                    ...
                    >
                        <Link
                            //@ts-ignore
                            to={`/hr/attendance/${row.original.id}`}
                        >
