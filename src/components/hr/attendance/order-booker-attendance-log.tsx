...
import { User, Briefcase, Hash, TrendingUp, HandCoins, Receipt, MapPin, Zap } from "lucide-react";
...
export const OrderBookerAttendanceLog = ({
  ...
  const stats = {
    totalSales: records.reduce((acc: number, r: any) => acc + (parseFloat(r.saleAmount || "0") || 0), 0),
    totalRecovery: records.reduce((acc: number, r: any) => acc + (parseFloat(r.recoveryAmount || "0") || 0), 0),
    totalPetrol: records.reduce((acc: number, r: any) => acc + (parseFloat(r.petrolAmount || "0") || 0), 0),
    totalDynamicTA: records.reduce((acc: number, r: any) => {
      if (r.isCompanyVehicle) return acc;
      if (r.paymentMode === "per_km") {
        return acc + ((parseFloat(r.distanceKm || "0") * parseFloat(r.perKmRate || "0")) || 0);
      }
      return acc;
    }, 0),
  ...
  const columns: ColumnDef<DayRow>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-semibold text-xs">{format(row.original.day, "dd MMM yyyy")}</span>
          <span className="text-[10px] text-muted-foreground">{format(row.original.day, "EEEE")}</span>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (record) {
          ...
          return (
            <Badge variant="outline" className={cn("text-[10px] font-bold uppercase px-2 py-0", statusColors[record.status] || "")}>
              {record.status.replace("_", " ")}
            </Badge>
          );
        }
        if (isWeekend) return <span className="text-[10px] text-muted-foreground font-semibold uppercase">Weekend</span>;
        return <Badge variant="outline" className="text-[10px] border-slate-300 text-slate-500 font-bold uppercase px-2 py-0">Pending</Badge>;
      },
    },
    {
      id: "area",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return r.isCompanyVehicle ? (
          <Badge variant="outline" className="bg-indigo-50 text-indigo-700 border-indigo-200 gap-1 text-[9px] font-black uppercase">
            <HugeiconsIcon icon={Cpu} className="size-4" />
          </Badge>
        ) : (
          <Badge variant="outline" className="bg-slate-50 text-slate-600 border-slate-200 gap-1 text-[9px] font-black uppercase">
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (r.isCompanyVehicle) {
          ...
          return (
            <div className="flex flex-col">
              <span className="text-indigo-700 font-black text-xs tabular-nums">PKR {petrol.toLocaleString()}</span>
              <span className="text-[9px] text-muted-foreground uppercase font-bold">Fuel Reimbursement</span>
            </div>
          );
        }
        ...
        const amount = (parseFloat(r.distanceKm || "0") * parseFloat(r.perKmRate || "0")).toFixed(0);
        return (
          <div className="flex flex-col">
            <span className="text-emerald-700 font-bold text-xs tabular-nums">PKR {amount}</span>
            <span className="text-[9px] text-muted-foreground uppercase">{r.distanceKm}km @ {r.perKmRate}/km</span>
          </div>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (sale === 0) return <span className="text-muted-foreground text-xs">—</span>;
        return <span className="text-emerald-700 font-bold text-xs tabular-nums">Rs {sale.toLocaleString()}</span>;
      },
    },
    {
      id: "recovery",
      ...
      cell: ({ row }) => {
        ...
        if (rec === 0) return <span className="text-muted-foreground text-xs">—</span>;
        return <span className="text-indigo-700 font-bold text-xs tabular-nums">Rs {rec.toLocaleString()}</span>;
      },
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch">
        ...
        <div className={cn("grid grid-cols-2 lg:grid-cols-5 gap-4", showHeader ? "md:col-span-6 lg:col-span-8" : "md:col-span-12")}>
          <StatCard color="emerald" icon={HandCoins} label="Total Sale" value={stats.totalSales.toLocaleString()} prefix="Rs" />
          <StatCard color="blue" icon={Receipt} label="Total Recovery" value={stats.totalRecovery.toLocaleString()} prefix="Rs" />
          <StatCard color="amber" icon={MapPin} label="Personal TA" value={stats.totalDynamicTA.toLocaleString()} prefix="PKR" />
          <StatCard color="indigo" icon={Zap} label="Petrol Reimb." value={stats.totalPetrol.toLocaleString()} prefix="PKR" />
          <StatCard color="emerald" icon={TrendingUp} label="Attendance" value={String(stats.daysPresent)} suffix="days" />
        </div>
      </div>

      <DataTable columns={columns} data={tableRows} showSearch={false} showViewOptions={false} pageSize={7} />
    </div>
  );
};
...
const statColorMap: Record<StatColor, any> = {
  ...
  indigo: { bg: "bg-indigo-50/60 dark:bg-indigo-950/20 border-indigo-200/60", iconBg: "bg-indigo-100 dark:bg-indigo-900/40", icon: "text-indigo-600", value: "text-indigo-700 dark:text-indigo-400", text: "text-indigo-600/70" },
};
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-2xl border p-4 transition-all hover:shadow-md", c.bg)}>
      ...
      <p className={cn("text-2xl font-black tracking-tight leading-tight", c.value)}>
        {prefix && <span className="text-sm font-bold mr-1 opacity-60">{prefix}</span>}
        {value}
        {suffix && <span className="text-sm font-bold ml-1 opacity-60">{suffix}</span>}
      </p>
    </div>
  );
}
