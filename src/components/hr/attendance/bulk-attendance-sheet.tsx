...
export const BulkAttendanceSheet = ({
    ...
    const [leaveType, setLeaveType] = useState<"sick" | "annual" | "special" | null>(null);
    ...
    const { data: preview, isFetching: previewLoading } = useBulkAttendancePreview(
        previewInput,
        !!previewInput,
    );
                ...
                <div className="py-5 space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        {startDate && endDate && endDate < startDate && (
                            <p className="text-[11px] text-destructive flex items-center gap-1">
                                <AlertCircle className="size-3" /> End date must be after start date
                            </p>
                        )}
                    </div>
                            ...
                            <div className="space-y-1.5 animate-in fade-in duration-200">
                                ...
                                <div className="flex items-start gap-2 p-2.5 rounded-lg border border-amber-200/60 bg-amber-50/50 dark:border-amber-900/40 dark:bg-amber-950/20">
                                    ...
                                    <p className="text-[11px] text-amber-700/80 dark:text-amber-500/80 leading-relaxed">
                                        All bulk-marked leaves are set to <strong>Pending</strong> and require individual admin approval.
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        {status === "present" && (
                            <div className="space-y-3 animate-in fade-in duration-200">
                                ...
                                <div>
                                    <p className="text-[12px] font-medium text-foreground/80 mb-2">
                                        Secondary Shift{" "}
                                        <span className="text-muted-foreground font-normal ml-1">(optional split)</span>
                                    </p>
                                    <div className="grid grid-cols-2 gap-3">
                                        <TimeField
                                            label="Check-In 2"
                                ...
                                {!checkIn && !checkOut && (
                                    <p className="text-[11px] text-muted-foreground flex items-center gap-1.5">
                                        ...
                                        No times entered — each employee's standard duty hours will be used.
                                    </p>
                                )}
                            </div>
                        )}
                    ...
                    <div className="space-y-3">
                        ...
                        <label className={cn(
                            ...
                            />
                            <div>
                                <p className={cn("text-[13px] font-semibold", selectAll ? "text-primary" : "text-foreground")}>
                                                ...
                                                {(emp.restDays ?? [0]).length > 0 && (
                                                    <div className="flex items-center gap-0.5 shrink-0">
                                                        ...
                                                        <span className="text-[9px] text-slate-400">
                                                            {(emp.restDays ?? [0]).map((d) => ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][d]).join(",")}
                                                        </span>
                                                    </div>
                                                )}
                                            </label>
                    ...
                    {previewInput && (
                        <div className={cn(
                            ...
                            {preview && preview.willCreate + preview.willUpdate === 0 && !previewLoading && (
                                <div className="flex items-center gap-2 text-[12px] text-muted-foreground">
                                    <Info className="size-3.5 shrink-0" />
                                    Nothing to apply — all dates are either rest days or already have records.
                                    {conflictStrategy === "skip" && preview.skippedExisting > 0 && (
                ...
                <AlertDialogContent>
                            ...
                            <div className="space-y-3">
                                ...
                                {preview && (
                                    <div className="bg-muted/40 rounded-lg p-3 space-y-1.5 text-sm">
                                        ...
                                        <div className="flex justify-between">
                                            <span className="text-muted-foreground">Records to create</span>
                                            <span className="font-bold text-emerald-600">{preview.willCreate}</span>
                                        </div>
                                        {preview.willUpdate > 0 && (
                                            <div className="flex justify-between">
                                                <span className="text-muted-foreground">Records to overwrite</span>
                                                <span className="font-bold text-blue-600">{preview.willUpdate}</span>
                                            </div>
                                        )}
                                        <div className="flex justify-between">
                                            <span className="text-muted-foreground">Employees affected</span>
                                            <span className="font-bold">{preview.totalEmployees}</span>
                                        </div>
                                    </div>
                                )}
                                {conflictStrategy === "overwrite" && preview && preview.willUpdate > 0 && (
                                    <p className="text-destructive text-xs flex items-center gap-1.5">
                                        <AlertCircle className="size-3.5 shrink-0" />
                                        {preview.willUpdate} existing records will be permanently replaced.
                                    </p>
                                )}
                                <p className="text-xs text-muted-foreground">
                                    This action cannot be undone. Individual payslip records are not affected.
                                </p>
                            </div>
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={handleConfirm} disabled={mutation.isPending}>
                            {mutation.isPending ? (
                                <><Loader2 className="size-4 mr-1.5 animate-spin" />Applying…</>
                            ) : (
                                "Confirm & Apply"
                            )}
                        </AlertDialogAction>
