...
import { Select, SelectContent, SelectTrigger, SelectValue, SelectItem as SelectOption } from "@/components/ui/select";
import { Textarea } from "@/components/ui/textarea";
import { useUpsertAttendance } from "@/hooks/hr/use-upsert-attendance";
import { Loader2, Clock, StickyNote, Info, Cpu, PenLine, AlertCircle, CheckCircle2 } from "lucide-react";
...
export const EditAttendanceForm = ({ employee, attendance, date, onSuccess }: Props) => {
    ...
    const form = useForm({
        defaultValues: {
            ...
            leaveType: ((attendance as any)?.leaveType ?? null) as "sick" | "casual" | "annual" | "unpaid" | "special" | null,
    ...
    return (
        ...
        >
            <FieldGroup>
                    ...
                    {(status) => status === "leave" && (
                        <div className="space-y-3 animate-in fade-in">
                            ...
                            <form.Field name="leaveType">
                                {(field) => (
                                    <Field>
                                        <FieldLabel>Leave Type <span className="text-destructive">*</span></FieldLabel>
                                        <Select
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                <SelectOption value="casual">Casual Leave (balance tracked)</SelectOption>
                                                <SelectOption value="annual">Annual Leave (balance tracked)</SelectOption>
                                                <SelectOption value="sick">Sick Leave (no deduction; Bradford counted)</SelectOption>
                                                <SelectOption value="special">Special Leave (Bradford counted)</SelectOption>
                                                <SelectOption value="unpaid">Unpaid Leave (conveyance deducted)</SelectOption>
                                            </SelectContent>
                                        </Select>
                                        <FieldError errors={field.state.meta.errors} />
                                    </Field>
                            ...
                            <form.Field name="isApprovedLeave">
                                {(field) => (
                                    <div className="flex items-start gap-3 p-3 rounded-lg border bg-indigo-50/40 dark:bg-indigo-900/10">
                                        ...
                                        />
                                        <div>
                                            <FieldLabel htmlFor="isApprovedLeave" className="mb-0 cursor-pointer text-indigo-700 dark:text-indigo-400 font-bold text-xs uppercase tracking-tight">
                                                Admin-Approved Paid Leave
                                            </FieldLabel>
                                            <p className="text-[11px] text-muted-foreground mt-0.5">
                                                Tick if admin-approved. No salary deduction. Sick leave is always no-deduction regardless.
                                            </p>
                                        </div>
                                    </div>
                                )}
                            ...
                            <div className="space-y-4 animate-in slide-in-from-top-2">
                                    ...
                                    {(field) => (
                                        <Field>
                                            <FieldLabel className="flex items-center gap-1.5">
                                                <Cpu className="size-3.5 text-muted-foreground" /> Entry Source
                                            </FieldLabel>
                                            <Select value={field.state.value} onValueChange={(v: any) => field.handleChange(v)}>
                                                <SelectTrigger className="h-9">
                                                    <SelectValue />
                                                ...
                                                <SelectContent>
                                                    <SelectOption value="biometric">
                                                        <span className="flex items-center gap-2"><Cpu className="size-3.5" /> Biometric Machine</span>
                                                    </SelectOption>
                                                    <SelectOption value="manual">
                                                        <span className="flex items-center gap-2"><PenLine className="size-3.5" /> Manual Entry</span>
                                                    </SelectOption>
                                                </SelectContent>
                                            </Select>
                                            <FieldError errors={field.state.meta.errors} />
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <form.Field name="checkIn2">
                                        {(field) => (
                                            <Field>
                                                <FieldLabel>Shift 2 – Start <span className="text-muted-foreground text-[10px]">(optional)</span></FieldLabel>
                                                <Input type="time" value={field.state.value || ""} onChange={(e) => field.handleChange(e.target.value || null)} />
                                            </Field>
                                        )}
                                    </form.Field>
                                    <form.Field name="checkOut2">
                                        {(field) => (
                                            <Field>
                                                <FieldLabel>Shift 2 – End <span className="text-muted-foreground text-[10px]">(optional)</span></FieldLabel>
                                                <Input type="time" value={field.state.value || ""} onChange={(e) => field.handleChange(e.target.value || null)} />
                                            </Field>
                                        )}
                                    </form.Field>
                                ...
                                <form.Subscribe selector={(s) => parseFloat(s.values.overtimeHours || "0") > 0}>
                                    {(hasOT) => hasOT && (
                                        <div className="space-y-3 p-3 rounded-lg border border-amber-200 bg-amber-50/40 dark:border-amber-800 dark:bg-amber-900/10 animate-in fade-in">
                                            <div className="flex items-center gap-2 text-amber-700 dark:text-amber-400 font-semibold text-xs uppercase tracking-tight">
                                                <AlertCircle className="size-3.5" /> Overtime Approval Required
                                            </div>

                                            <form.Field name="overtimeRemarks">
                                                {(field) => (
                                                    <Field>
                                                        <FieldLabel>Overtime Reason <span className="text-destructive">*</span></FieldLabel>
                                                        <Textarea
                                            ...
                                            <form.Field name="overtimeStatus">
                                                {(field) => (
                                                    <Field>
                                                        <FieldLabel className="flex items-center gap-1.5">
                                                            <CheckCircle2 className="size-3.5" /> Approval Status
                                                        </FieldLabel>
                                                        <Select value={field.state.value || "pending"} onValueChange={(v: any) => field.handleChange(v)}>
                                                            <SelectTrigger className="h-9">
                                                                <SelectValue />
                                                            </SelectTrigger>
                                                            <SelectContent>
                                                                <SelectOption value="pending">Pending Admin Approval</SelectOption>
                                                                <SelectOption value="approved">Approved</SelectOption>
                                                                <SelectOption value="rejected">Rejected</SelectOption>
                                                            </SelectContent>
                                                        </Select>
                                                        <p className="text-[11px] text-muted-foreground">
                                                            Overtime pay is only calculated for <strong>Approved</strong> records.
                                                        </p>
                                                    </Field>
                                                )}
                                            </form.Field>
                                        ...
                                        {(field) => (
                                            <div className="flex items-center gap-2 p-2.5 rounded-lg border bg-rose-50/30 dark:bg-rose-900/10 flex-1">
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={(c) => field.handleChange(!!c)}
                                                />
                                                <FieldLabel htmlFor="isLateEdit" className="mb-0 cursor-pointer text-rose-700 dark:text-rose-400 font-bold text-xs uppercase tracking-tight">
                                                    Late Arrival
                                                </FieldLabel>
                                            </div>
                                        )}
                                        ...
                                        {(field) => (
                                            <div className="flex items-center gap-2 p-2.5 rounded-lg border bg-indigo-50/30 dark:bg-indigo-900/10 flex-1">
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={(c) => field.handleChange(!!c)}
                                                />
                                                <FieldLabel htmlFor="isNightShiftEdit" className="mb-0 cursor-pointer text-indigo-700 dark:text-indigo-400 font-bold text-xs uppercase tracking-tight">
                                                    Night Shift
                                                </FieldLabel>
                                            </div>
                                        )}
                ...
                <div className="pt-2">
                    <Button type="submit" disabled={form.state.isSubmitting} className="w-full h-11">
                        {form.state.isSubmitting ? <Loader2 className="mr-2 size-4 animate-spin" /> : "Save Attendance Record"}
                    </Button>
                </div>
            </FieldGroup>
        </form>
