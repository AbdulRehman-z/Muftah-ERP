    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            if (value.password && value.password.length >= 8) {
                promises.push(setUserPassword.mutateAsync({
                    userId: user.id,
        ...
        >
            <div className="flex flex-col gap-2 text-sm text-muted-foreground p-3 border rounded-md bg-muted/20">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <Badge variant="secondary" className="capitalize px-2 py-0 h-5 text-[10px] font-bold tracking-tight">
                        {user.role.replace("-", " ")}
                    </Badge>
                </div>
                <div className="flex justify-between items-center">
                    <span className="font-medium text-foreground">Status</span>
                    <Badge variant={user.banned ? "destructive" : "outline"} className={!user.banned ? "text-green-600 border-green-200 bg-green-50 h-5 text-[10px] font-bold" : "h-5 text-[10px] font-bold"}>
