...
import {
    Building2, BanknoteIcon, TrendingDown, Tag, Hash,
...
export const ExpensesContainer = () => {
    ...
    const columns: ColumnDef<Expense>[] = useMemo(() => [
        ...
        },
        {
            ...
            header: "Description",
            cell: ({ row }) => <span className="font-bold text-sm">{row.original.description}</span>,
        },
        {
            accessorKey: "category",
            header: "Category",
            cell: ({ row }) => (
                <Badge variant="outline" className="rounded-none text-[10px] font-bold tracking-wider border-border">
        ...
        },
        {
            ...
            header: "Recorded By",
            cell: ({ row }) => (
                <span className="text-sm text-muted-foreground">{row.original.performer?.name || "—"}</span>
            ),
        },
        {
            accessorKey: "amount",
    ...
    return (
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6 font-sans antialiased">
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <SharpKPICard title="Total Records" value={total.toString()} subtext={categoryFilter !== "all" ? `Filtered by ${categoryFilter}` : "All categories"} icon={Hash} theme="blue" />
                <SharpKPICard title="Page Total" value={`₨ ${Math.round(pageTotal).toLocaleString()}`} subtext={`${expenses.length} records on this page`} icon={TrendingDown} theme="amber" />
                <SharpKPICard title="Categories" value={isLoadingCategories ? "..." : categories.length.toString()} subtext="Available expense categories" icon={Tag} theme="rose" />
            </motion.div>
                ...
                <DataTable
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                            <Dialog open={isManageCategoriesOpen} onOpenChange={setIsManageCategoriesOpen}>
                                <DialogTrigger asChild>
                                    <Button variant="outline" className="h-10 rounded-none shadow-none border-border hover:bg-muted transition-colors">
                            ...
                            <Select value={categoryFilter} onValueChange={handleCategoryFilter}>
                                ...
                                <SelectContent className="rounded-none shadow-none border border-border">
                                    ...
                                    {categories.map((cat: any) => (
                                        <SelectItem key={cat.id} value={cat.name} className="rounded-none">{cat.name}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        </div>
                    ...
                    emptyState={
                        <GenericEmpty
                            ...
                            description={categoryFilter !== "all" ? `No expenses in "${categoryFilter}".` : "No expenses recorded yet."}
                        />
                    }
                />
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center justify-between px-1 gap-4">
                <span className="text-xs text-muted-foreground tabular-nums uppercase tracking-widest font-semibold">
                    Page <span className="text-foreground">{page}</span> of <span className="text-foreground">{pageCount}</span> · {total} total records
                </span>
