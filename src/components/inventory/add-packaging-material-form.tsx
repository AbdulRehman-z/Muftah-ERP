    ...
    <div className="space-y-4 w-full">
      ...
      <div className="grid grid-cols-3 gap-2 p-1 bg-muted rounded-lg border">
        ...
        >
          <Info
            className={cn(
              "size-3.5",
              activeType === "sticker" ? "text-white" : "text-muted-foreground",
            )}
          />
          Sticker
        </Button>
      ...
      >
        ...
        <FieldGroup>
                ...
                {(field) => (
                  <Field>
                    ...
                    <Input
                      ...
                      onChange={(e) => field.handleChange(e.target.value)}
                      placeholder={
                        activeType === "primary" ? "e.g. 500" : "e.g. 24"
                      }
                    />
                    <FieldDescription>
                      {activeType === "primary"
              ...
              <form.Field name="weightPerPack">
                {(field) => (
                  <Field>
                    ...
                    />
                    <FieldDescription>
                      Weight of the empty bottle/wrapper.
                    </FieldDescription>
                    <FieldError errors={field.state.meta.errors} />
                  </Field>
                )}
              ...
              <form.Field name="pricePerKg">
                {(field) => (
                  <Field>
                    ...
                    />
                    <FieldDescription>
                      Purchase price per kg of the material (e.g., wrapper/film
                      cost).
                    </FieldDescription>
                    <FieldError errors={field.state.meta.errors} />
                  </Field>
          ...
          <form.Subscribe
            selector={(state) => [
              state.values.weightPerPack,
              state.values.pricePerKg,
              state.values.type,
            ]}
                ...
                return (
                  <div className="flex flex-col gap-3 bg-primary/5 p-4 rounded-xl border border-primary/10  animate-in fade-in slide-in-from-top-2">
                    ...
                    <p className="text-[10px] text-muted-foreground italic border-t pt-2 border-primary/5">
                      Matches the spreadsheet logic: {w}g is{" "}
                      {(w / 1000).toFixed(3)}kg. Total = {(w / 1000).toFixed(3)}{" "}
                      × {p} = {exactCost.toFixed(2)}.
                    </p>
                  </div>
                );
              }
