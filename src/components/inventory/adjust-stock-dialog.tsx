...
export const AdjustStockDialog = ({
  ...
  const handleSubmit = () => {
    ...
    if (mode === "remove" && qty > currentStock) {
      toast.error(
        `Cannot remove ${qty} ${unit}. Only ${currentStock.toFixed(2)} ${unit} available.`,
      );
      return;
    }
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            floor. This is for disaster recovery (machine breakdown, spillage,
            etc.).
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="text-xs text-muted-foreground p-3 bg-amber-50/50 border border-amber-100 rounded-md">
            ⚠️ All manual adjustments are <strong>permanently logged</strong> in
            the audit trail with your name and reason.
          </div>
        </div>
