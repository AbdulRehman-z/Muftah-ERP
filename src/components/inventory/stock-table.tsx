...
export const StockTable = ({
	...
	const selectedWarehouse = warehouses.find(w => w.id === preselectedWarehouse);
	const isAddAllowed = !selectedWarehouse || selectedWarehouse.type === "factory_floor";
		...
		return (
			<>
				<GenericEmpty
					...
					title={selectedWarehouse?.type === "storage" ? "Invalid Storage Type" : `No ${isChemical ? "Chemicals" : "Packaging Material"} Stock`}
					description={description}
					ctaText={isAddAllowed && !hideAddButton ? `Add ${isChemical ? "Chemicals" : "Packaging Material"}` : undefined}
					onAddChange={isAddAllowed && !hideAddButton ? setCreateDialogOpen : undefined}
				/>
			...
			<div className="flex justify-between items-center mb-6">
				<div>
					...
					<p className="text-sm text-muted-foreground">
						Manage and monitor your {isChemical ? "ingredient" : "packaging"} stock levels.
					</p>
				</div>
