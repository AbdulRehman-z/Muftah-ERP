...
export const ProductionRunsContainer = () => {
	...
	const filteredRuns = runs.filter(
		(run) => {
			// Search Filter
			const matchesSearch = run.batchId.toLowerCase().includes(searchQuery.toLowerCase()) ||
			...
			if (dateFilter !== "all") {
				...
				if (dateFilter === "last_7_days" && !isAfter(runDate, startOfDay(subDays(new Date(), 7)))) return false;
			}
					...
					<CardContent>
						<div className="text-2xl font-bold">
							...
							{filteredRuns
								...
								.reduce((sum, r) => sum + parseFloat(r.totalProductionCost || "0"), 0)
					...
					<CardContent>
						<div className="text-2xl font-bold">
							...
							{(() => {
								...
								const totalCost = completedRuns.reduce((sum, r) => sum + parseFloat(r.totalProductionCost || "0"), 0);
								const totalContainers = completedRuns.reduce((sum, r) => sum + r.containersProduced, 0);
								return totalContainers > 0 ? (totalCost / totalContainers).toFixed(2) : "0.00";
							})()}
						</div>
					</CardContent>
				</Card>
