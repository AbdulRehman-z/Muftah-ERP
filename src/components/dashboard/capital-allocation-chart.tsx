...
export function CapitalAllocationChart({ rawStock, finishedStock, payroll, expenses, className }: any) {
    ...
    const chartData = hasData ? rawData.filter(d => d.value > 0) : [
        ...
        <div className={cn("relative border border-border/60 bg-card flex flex-col rounded-2xl  overflow-hidden", className)}>
            ...
            <div className="px-5 py-5 border-b border-border/50 flex items-center justify-between bg-card/80 backdrop-blur-xl relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-sm font-black uppercase tracking-tighter text-foreground leading-none">Capital Vector</h3>
                        <p className="text-[10px] font-bold text-muted-foreground mt-1.5 font-mono uppercase tracking-widest">Asset_Allocation_Probe</p>
                    </div>
                </div>
                {!hasData && (
                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-amber-500/10 border border-amber-500/20">
                        <Activity className="size-3 text-amber-500 animate-pulse" />
                        <span className="text-[8px] font-bold text-amber-500 uppercase font-mono">Ghost_Mode</span>
                    </div>
                )}
            </div>
            ...
            <div className="flex-1 p-5 flex flex-col justify-center relative bg-background/20">
                ...
                <div className="h-[280px] w-full relative z-10">
                                        ...
                                        return (
                                            <div className="border border-primary/20 bg-background/90 backdrop-blur-xl p-3 rounded-xl shadow-sm">
                                                <p className="text-[10px] font-black uppercase text-muted-foreground mb-2 flex items-center gap-2">
                                                    <span className="size-2 rounded-full" style={{ backgroundColor: data.color }} /> {data.name}
                                                </p>
                                                <p className="text-sm font-black tabular-nums text-foreground">PKR {data.value.toLocaleString()}</p>
                                            </div>
                                        )
                                    }}
                                />
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground font-mono opacity-60">Total_Asset_Value</span>
                ...
                <div className="grid grid-cols-2 gap-px mt-8 bg-border/20 border border-border/50 rounded-xl overflow-hidden">
                    {rawData.map((item) => (
                        <div key={item.name} className="flex items-center justify-between p-3 bg-card/40">
                            <div className="flex items-center gap-2.5">
                                <div className={cn("size-2 rounded-sm shrink-0", !hasData && "bg-muted")} style={{ backgroundColor: hasData ? item.color : undefined }} />
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter truncate">{item.name}</span>
                            </div>
