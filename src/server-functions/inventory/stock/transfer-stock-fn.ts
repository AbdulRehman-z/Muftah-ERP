  ...
  .handler(async ({ data, context }) => {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (data.materialType === "finished") {
        ...
        if (
          sourceStock.quantityCartons < qty ||
          sourceStock.quantityContainers < loose
        ) {
          throw new Error(
            `Insufficient stock. Available: ${sourceStock.quantityCartons} cartons, ${sourceStock.quantityContainers} units.`,
          );
        }
