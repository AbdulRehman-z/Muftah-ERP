	...
	.handler(async ({ data, context }) => {
		return await db.transaction(async (tx) => {
			...
			if (warehouse.type !== "factory_floor") {
				throw new Error("Raw materials (Chemicals/Packaging) must be added to a Factory Floor facility, not Storage.");
			}
			...
			const [purchase] = await tx.insert(purchaseRecords).values({
				...
				packagingMaterialId: data.materialType === "packaging" ? data.materialId : null,
				quantity: data.quantity,
				cost: data.cost,
