...
import { db, finishedGoodsStock, inventoryAuditLog, materialStock, productionRuns, stockTransfers, warehouses } from "@/db";
    ...
    .handler(async ({ data: { warehouseId } }) => {
        try {
            return await db.transaction(async (tx) => {
                ...
                if (runs) {
                    throw new Error("Cannot delete warehouse: It has associated production history. Deactivate it instead.");
                }
                ...
                if (transfers) {
                    throw new Error("Cannot delete warehouse: It has associated stock transfer records. Deactivate it instead.");
                }

                // 3. Clear transient stock data
                await tx.delete(materialStock).where(eq(materialStock.warehouseId, warehouseId));
                await tx.delete(finishedGoodsStock).where(eq(finishedGoodsStock.warehouseId, warehouseId));

                // 4. Clear audit logs (optional, but needed for clean hard delete)
                await tx.delete(inventoryAuditLog).where(eq(inventoryAuditLog.warehouseId, warehouseId));
            ...
            throw new Error(error.message || "Failed to permanently delete warehouse");
        }
    });
