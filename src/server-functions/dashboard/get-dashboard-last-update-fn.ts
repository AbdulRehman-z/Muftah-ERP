...
export const getDashboardLastUpdateFn = createServerFn()
    ...
    .handler(async () => {
        ...
        const [latestInvoice, latestRun, latestPayslip, latestExpense] = await Promise.all([
            db.select({ ts: invoices.createdAt }).from(invoices).orderBy(desc(invoices.createdAt)).limit(1),
            db.select({ ts: productionRuns.updatedAt }).from(productionRuns).orderBy(desc(productionRuns.updatedAt)).limit(1),
            db.select({ ts: payslips.createdAt }).from(payslips).orderBy(desc(payslips.createdAt)).limit(1),
            db.select({ ts: expenses.createdAt }).from(expenses).orderBy(desc(expenses.createdAt)).limit(1),
        ]);
