...
export const deletePurchaseRecordFn = createServerFn()
    ...
    .handler(async ({ data }) => {
        ...
        await db.transaction(async (tx) => {
            ...
            await tx.delete(supplierPayments).where(eq(supplierPayments.purchaseId, data.id));
            ...
            } else if (purchase.materialType === "packaging" && purchase.packagingMaterialId) {
                await tx
                    .update(materialStock)
                    .set({
                        quantity: sql`${materialStock.quantity} - ${purchase.quantity}`,
                    ...
                    .where(
                        and(
                            eq(materialStock.warehouseId, purchase.warehouseId),
                            eq(materialStock.packagingMaterialId, purchase.packagingMaterialId)
                        )
                    );
            }
