...
export const getSupplierDetailsFn = createServerFn()
    ...
    .handler(async ({ data }) => {
        ...
        const totalPurchases = supplier.purchases.reduce((acc, p) => acc + parseFloat(p.cost), 0);
        const totalPayments = supplier.payments.reduce((acc, p) => acc + parseFloat(p.amount), 0);
        ...
        const chemicalIds = new Set(supplier.purchases.map(p => p.chemicalId).filter(Boolean));
        const packagingIds = new Set(supplier.purchases.map(p => p.packagingMaterialId).filter(Boolean));
        ...
        let stockMap: Record<string, { current: number; min: number; warehouse: string }> = {};
        ...
        const enrichedPurchases = supplier.purchases.map(p => {
            ...
            const relatedPayments = supplier.payments.filter(pay => pay.purchaseId === p.id);
            const lastPayment = relatedPayments.length > 0 ? relatedPayments[0] : null; // Sorted by desc createdAt already
