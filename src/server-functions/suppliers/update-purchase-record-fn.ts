...
export const updatePurchaseRecordFn = createServerFn()
    ...
    .handler(async ({ data }) => {
        return await db.transaction(async (tx) => {
            ...
            if (qtyDiff !== 0) {
                const materialField = existingRecord.materialType === "chemical"
                    ? materialStock.chemicalId
                    : materialStock.packagingMaterialId;

                const materialId = existingRecord.materialType === "chemical"
            ...
            if (existingRecord.materialType === "packaging" && existingRecord.packagingMaterialId) {
                await tx.update(packagingMaterials)
                    .set({
                        name: data.materialName || undefined,
                        capacity: data.capacity || undefined,
                        capacityUnit: data.capacityUnit || undefined,
                        minimumStockLevel: data.minStock ? parseInt(data.minStock) : undefined,
                        updatedAt: new Date(),
                    })
            ...
            if (data.quantity && data.cost) {
                ...
                updateData.unitCost = (parseFloat(data.cost) / parseFloat(data.quantity)).toFixed(4);
            }
