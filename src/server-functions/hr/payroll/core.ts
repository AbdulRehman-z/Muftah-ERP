...
export async function generateEmployeePayslipCore(input: GeneratePayslipInput) {
  ...
  const [savedPayslip] = await db
    ...
    .values({
      ...
      nightShiftAllowanceAmount:
        payslipCalc.nightShiftAllowanceAmount.toString(),
  ...
  if (autoUpdateLeaveBalances) {
    ...
    const annualTaken = leaveTaken.filter(
      (r) => r.leaveType === "annual",
    ).length;
    const casualTaken = leaveTaken.filter(
      (r) => r.leaveType === "casual",
    ).length;
      ...
      await db
        ...
        .set({
          annualLeaveBalance: Math.max(
            0,
            (employeeData.annualLeaveBalance ?? 30) - annualTaken,
          ),
          casualLeaveBalance: Math.max(
            0,
            (employeeData.casualLeaveBalance ?? 5) - casualTaken,
          ),
          sickLeaveBalance: Math.max(
            0,
            (employeeData.sickLeaveBalance ?? 10) - sickTaken,
          ),
        })
        .where(eq(employees.id, employeeId));
    }
