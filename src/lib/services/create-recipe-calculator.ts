...
export function calculateRecipeCost(recipe: RecipeWithDetails) {
	...
	const chemicalCost = recipe.ingredients.reduce((sum, ing) => {
		return (
			sum +
			parseFloat(ing.quantityPerBatch) * parseFloat(ing.chemical.costPerUnit || "0")
		);
	}, 0);
	...
	const sizeUnit = containerSize.replace(sizeValue.toString(), "").trim().toLowerCase();
	...
	} else if (recipe.batchUnit === "kg" && sizeUnit.includes("g") && !sizeUnit.includes("kg")) {
		containerSizeInBatchUnit = sizeValue / 1000;
	}
	...
	const additionalPackagingCost = recipe.packaging.reduce((sum, pkg) => {
		...
		return sum + qtyNeeded * parseFloat(pkg.packagingMaterial.costPerUnit || "0");
	}, 0);

	// Carton cost
	const totalCartons = recipe.containersPerCarton ? Math.floor(totalContainers / recipe.containersPerCarton) : 0;
