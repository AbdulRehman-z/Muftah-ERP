...
import { calculatePayslip, type EmployeeData, type AttendanceRecord } from "@/lib/payroll-calculator";
...
async function seedHR() {
    ...
    for (const emp of employeeData) {
        ...
        if (existing) {
            ...
            console.log(`  Updated: ${emp.employeeCode} – ${emp.firstName} ${emp.lastName}`);
        } else {
            ...
            console.log(`  Inserted: ${emp.employeeCode} – ${emp.firstName} ${emp.lastName}`);
        }
    }
    ...
    for (const date of dateRange) {
        ...
        for (const emp of insertedEmployees) {
            ...
            if (status !== "present") {
                ...
                const leaveTypes = ["casual", "sick", "annual", "unpaid", "special"] as const;
    ...
    for (const emp of insertedEmployees) {
        ...
        const periodAttendance: AttendanceRecord[] = attRecords
            ...
            .map(r => ({
                ...
                overtimeStatus: (r.overtimeStatus as "approved" | "pending" | "rejected") ?? "pending",
            }));
    ...
    const totalGross = allPayslips.reduce((s, p) => s + parseFloat(p.grossSalary ?? "0"), 0);
